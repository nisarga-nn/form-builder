...
import Show from "./src/listing/Show.jsx";
...
export default class Demobar extends React.Component {
  ...
  //   const response = await axios.get('http://localhost:5005/api/onlydata');
  //   console.log("Returning the data", response.data);
  //   console.log("Returning the data", store.state.data)

  //   try {
  //     const postResponse = await axios.post('http://localhost:5005/api/updatedata', store.state.data);
  ...
  async handleEdit()  {
    try{
      let response = await fetch("http://localhost:5005/api/updateForm/641c2ae47fc041b16768c79f", { 
  method: "PATCH",
  body: JSON.stringify({
...
console.log(data);
  ...
  async handleSave() {
    try{
      let response = await fetch("http://localhost:5005/api/formdata", { 
      method: "POST",
      body: JSON.stringify({
        name: "hello world",
        task_data: store.state.data
    ...
    return (
      <div className="clearfix" style={{ margin: "10px", width: "70%" }}>
        ...
        <button
          className="btn btn-default float-right"
          style={{ marginRight: "10px" }}
          onClick={() => this.saveFormData("save")}
        >
          Save Form
        </button>
        <button
          className="btn btn-default float-right"
          style={{ marginRight: "10px" }}
          onClick={() => this.saveFormData("edit")}
        >
          Update Form
        </button>
